...
def migrate_trip_refinement_requests_schema(bind: Any) -> dict[str, Any]:
    """Create the refinement operation ledger for databases initialized before this feature."""
    def operation(conn):
        if conn.execute(text("SELECT to_regclass('public.trip_refinement_requests')")).scalar() is not None:
            return {"created": False, "verified": True}
        conn.execute(text("""CREATE TABLE public.trip_refinement_requests (
            ...
            CONSTRAINT uq_trip_refinement_owner_operation UNIQUE (user_id, operation_id)
        )"""))
